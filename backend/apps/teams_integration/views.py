...
def _get_or_create_meeting(teams_meeting_id: str, subject: str | None = None):
    meeting, created = TeamsMeeting.objects.get_or_create(
        teams_meeting_id=teams_meeting_id,
        defaults={"subject": subject or teams_meeting_id},
    )
    if subject and meeting.subject != subject:
        meeting.subject = subject
        meeting.save(update_fields=["subject"])
    return meeting
...
def teams_bot_webhook(request):
    ...
    meeting = _get_or_create_meeting(teams_meeting_id, subject=subject)
